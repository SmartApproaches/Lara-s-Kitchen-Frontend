...
const DineInGeoFence = () => {
  ...
  const {
    ...
    error: errorGeoFence,
  } = useGetGeofenceByTypeQuery("dine_in");
  ...
  useEffect(() => {
    ...
    if (errorGeoFence) {
      toast.error(errorGeoFence?.message || "Failed to load geofence");
    }
  }, [isErrorPost, errorGeoFence]);
  ...
  const handleSave = async () => {
    ...
    try {
      if (updateGeofenceData) {
        await updateGeofence({ type: "dine_in", data: payload }).unwrap();
  ...
  const handleDeleteConfirm = async () => {
    try {
      await deleteGeofence("dine_in").unwrap();
